...
export const ParametersList = ({ userInfo }) => {
  ...
  return (
    <div
      className="absolute right-0 top-[100%] mt-[10px] max-w-[450px] rounded-2xl bg-black py-[10px] text-white"
      style={{
    ...
    >
      ...
      <ul>
        {userInfo.map((user) => (
          <li
            key={user.id}
            className={`flex cursor-pointer items-center gap-[20px] px-[20px] py-[10px] sm:hover:bg-white/5 ${
        ...
        <li className={`${styleItemList}`}>Перейти на інший обліковий запис</li>
      </ul>
      <h2 className="my-[10px] text-center text-[20px] font-bold">Параметри</h2>
      <ul>
